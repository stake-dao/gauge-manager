...
reward_queue: public(HashMap[address, uint256])
...
def _checkpoint_rewards(_user: address, _total_supply: uint256, _claim: bool, _receiver: address):
    ...
    for i in range(MAX_REWARDS):
        ...
        self._push_queued_rewards(token)
...
@internal
def _push_queued_rewards(_reward_token: address):
    """
    @notice push the rewards in reward_queue into the streaming rewards
    @param _reward_token The reward token being distributed
    """
    period_finish: uint256 = self.reward_data[_reward_token].period_finish
    # dev : reward_queue[_reward_token] > WEEK avoids rate to be 0 and rewards to be lost in gauge
    if block.timestamp >= period_finish and self.reward_queue[_reward_token] > WEEK:
        self.reward_data[_reward_token].rate = self.reward_queue[_reward_token] / WEEK
        self.reward_data[_reward_token].last_update = block.timestamp
        self.reward_data[_reward_token].period_finish = block.timestamp + WEEK  
        # To avoid lost tokens in the rounding of the rate, compute the total tokens distributed and keeps the leftover for next distribution
        self.reward_queue[_reward_token] -= self.reward_data[_reward_token].rate * WEEK 
...
def deposit_reward_token(_reward_token: address, _amount: uint256):
    ...
    self.reward_queue[_reward_token] += _amount
