...
interface ILiquidityGauge {
    ...
    function reward_data_count(address _reward_token) external view returns(uint256);
    ...
    function reward_data(address, uint256 index) external returns(Reward memory);
}
...
contract LiquidityGaugePermissionlessTest is Test {
    ...
    function setUp() public {
        ...
        liquidityGauge = ILiquidityGauge(deployCode("LiquidityGaugePermissionless2", abi.encode(address(this))));
    ...
    function testDepositRewardToken() public {
        ...
        assertEq(liquidityGauge.reward_data(rewardToken, 0).rate, depositRewardAmount / WEEK);
        assertEq(liquidityGauge.reward_data(rewardToken, 0).period_finish, block.timestamp + WEEK);
    }
    ...
    function testDepositRewardTokenMultipleDepositors() public {
        ...
        vm.startPrank(bob);
        IERC20(rewardToken).approve(address(liquidityGauge), depositRewardAmount);
        liquidityGauge.deposit_reward_token(rewardToken, depositRewardAmount, 2 * WEEK);
        vm.stopPrank();

        assertEq(liquidityGauge.reward_data(rewardToken, 0).rate, depositRewardAmount / WEEK);
        assertEq(liquidityGauge.reward_data(rewardToken, 0).period_finish, block.timestamp + WEEK);

        assertEq(liquidityGauge.reward_data(rewardToken, 1).rate, depositRewardAmount / (2 * WEEK));
        assertEq(liquidityGauge.reward_data(rewardToken, 1).period_finish, block.timestamp + 2 * WEEK);
    }
    ...
    function testDepositDifferentTimestamps() public {
        ...
        liquidityGauge.deposit_reward_token(rewardToken, 4 * depositRewardAmount, 2 * WEEK);
        vm.stopPrank();

        assertEq(liquidityGauge.reward_data(rewardToken, 0).rate, depositRewardAmount / WEEK);
        assertEq(liquidityGauge.reward_data(rewardToken, 0).period_finish, block.timestamp + 3 days);
    
        assertEq(liquidityGauge.reward_data(rewardToken, 1).rate, 4 * depositRewardAmount / (2 * WEEK));
        assertEq(liquidityGauge.reward_data(rewardToken, 1).period_finish, block.timestamp + 2 * WEEK);
    }
    ...
    function testClaimableAndClaim() public {
        ...
        // Bob deposits rewards into the gauge
        vm.startPrank(bob);
        IERC20(rewardToken).approve(address(liquidityGauge), 4 * depositRewardAmount);
        liquidityGauge.deposit_reward_token(rewardToken, 4 * depositRewardAmount, 2 * WEEK);
        vm.stopPrank();
